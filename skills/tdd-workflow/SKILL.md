...
When the task involves removing a feature, function, endpoint, component, or
any other existing behavior:

1. **Identify all tests that cover the behavior being removed.** Search for
   tests that exercise the functionality slated for deletion.
2. **Delete those tests.** Removal of behavior means removal of the tests
   that specify that behavior. Do not replace them with tests that assert
   the behavior is absent (e.g., do not write tests like "should throw
   NotFoundError when calling removedFunction").
3. **Run the full test suite.** Remaining tests should still pass. If any
   remaining test fails because it depended on the removed behavior, evaluate
   whether that test should also be removed or updated to reflect the new
   expected behavior.
4. **Remove the implementation code.**
5. **Run the full test suite again.** All remaining tests must pass.
6. **Commit.** Use the message format: `refactor: remove <what was removed>`.

Do NOT write new tests to assert that removed code is absent, throws errors,
or returns specific error responses. The absence of tests for that behavior
is itself the specification that the behavior no longer exists. The only
exception is if the removal changes an existing public API contract where
callers need a specific, well-defined error -- and that error behavior is
explicitly part of the new requirements.
...
| Writing tests to assert removed behavior is gone | Adds unnecessary tests that verify absence; clutters the test suite | Remove the tests that covered the deleted behavior |
